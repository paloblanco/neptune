...
A lightweight JavaScript 3D game engine for small devs with little to no 3D experience. Import some spritesheets and you are ready to go!

Neptune wraps Three.js entirely behind a clean, PICO-8-inspired API so you can build a complete game in a single `main.js` file without ever touching Three.js directly.
...
Common key names: `'ArrowLeft'`, `'ArrowRight'`, `'ArrowUp'`, `'ArrowDown'`, `'Enter'`, `'Escape'`, `'Shift'`, `'Control'`, `' '` (spacebar), `'a'`–`'z'`.
...
The underscore prefix is intentional — prefer Neptune's API wherever possible.
